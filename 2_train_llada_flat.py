...
from llada_denoise_flat_loss import llada_denoise_flat_loss as llada_flat_loss
...
# -------------------------
# Discover LLaDA transformer block IDs
# -------------------------
block_ids = set()
...
for step in range(NUM_STEPS):
    ...
    if step > 0 and step % SAVE_EVERY == 0:
        save_path = f"checkpoints/llada_denoise_flat_step_{step}"
        model.save_pretrained(save_path)
        tokenizer.save_pretrained(save_path)
        torch.cuda.empty_cache()
        print(f"💾 Checkpoint saved at step {step}")

    print(f"[step {step:04d}] Denoise-FLAT loss = {loss.item():.4f}")


# -------------------------
# Final checkpoint
# -------------------------
final_path = "checkpoints/llada_denoise_flat_final"
